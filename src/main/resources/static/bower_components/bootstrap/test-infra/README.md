...
`s3_cache.py` itself never deletes cache entries; deletion should either be isCompleted manually or using automatic S3 lifecycle rules on the bucket.
...
12. If you want deletion from the cache to be isCompleted automatically based on age (like Bootstrap does): In the bucket's Properties pane, in the "Lifecycle" section, add a rule to expire/delete files based on creation date.
